...
-- 管理员信息表
CREATE TABLE `admin` (
                         `admin_id` BIGINT NOT NULL PRIMARY KEY COMMENT '管理员唯一标识ID',
                         `password` VARCHAR(15) NOT NULL COMMENT '管理员登录密码',
                         `username` VARCHAR(15) DEFAULT NULL COMMENT '管理员用户名'
) ENGINE=INNODB DEFAULT CHARSET=utf8;

INSERT INTO `admin` VALUES (123456, '123456', 'admin');
-- 图书信息表
CREATE TABLE `book_info` (
                             `book_id` BIGINT NOT NULL PRIMARY KEY COMMENT '图书唯一标识ID',
                             `name` VARCHAR(20) NOT NULL COMMENT '图书名称',
                             `author` VARCHAR(15) NOT NULL COMMENT '作者',
                             `publish` VARCHAR(20) NOT NULL COMMENT '出版社',
                             `ISBN` VARCHAR(15) NOT NULL COMMENT '国际标准书号',
                             `introduction` text COMMENT '图书简介',
                             `language` VARCHAR(4) NOT NULL COMMENT '语言',
                             `price` DECIMAL(10,2) NOT NULL COMMENT '价格',
                             `pub_date` date NOT NULL COMMENT '出版日期',
                             `class_id` INT DEFAULT NULL COMMENT '分类ID（关联class_info表）',
                             `number` INT DEFAULT NULL COMMENT '馆藏数量'
) ENGINE=INNODB DEFAULT CHARSET=utf8;

ALTER TABLE `book_info`
    MODIFY `book_id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '图书唯一标识ID',
    AUTO_INCREMENT=1;
...
CREATE TABLE `class_info` (
                              `class_id` INT NOT NULL PRIMARY KEY COMMENT '分类唯一标识ID',
                              `class_name` VARCHAR(15) NOT NULL COMMENT '分类名称'
) ENGINE=INNODB DEFAULT CHARSET=utf8;
...
CREATE TABLE `lend_list` (
                             `ser_num` BIGINT NOT NULL PRIMARY KEY COMMENT '借阅流水号',
                             `book_id` BIGINT NOT NULL COMMENT '图书ID（关联book_info表）',
                             `reader_id` BIGINT NOT NULL COMMENT '读者ID（关联reader_info表）',
                             `lend_date` datetime DEFAULT NULL COMMENT '借出日期',
                             `back_date` datetime DEFAULT NULL COMMENT '归还日期（NULL表示未归还）'
) ENGINE=INNODB DEFAULT CHARSET=utf8;
ALTER TABLE `lend_list`
    MODIFY `ser_num` BIGINT NOT NULL AUTO_INCREMENT COMMENT '借阅流水号',
    AUTO_INCREMENT=1;
...
CREATE TABLE `reader_info` (
                               `reader_id` BIGINT NOT NULL PRIMARY KEY COMMENT '读者唯一标识ID',
...
ALTER TABLE `reader_info`
    MODIFY `reader_id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '读者唯一标识ID',
...
CREATE TABLE `reader_card` (
                               `reader_id` BIGINT NOT NULL PRIMARY KEY COMMENT '读者ID（关联reader_info表）',
